...
tardis <- trixy[as.character(aggy$word2[i])]
...
##print(paste("number of rows in trixy is ",nrow(trixy)))
    ...
    for (i in d:a)
{ 
tardis <- trixy[word2 == aggy$word2[i],]
tardis$prob <- (tardis$bi_gram_ns_ns - downstream)/aggy$sum[i]
mid_tri <- rbind(mid_tri, tardis)
...
combi_bi_ns_ns <- read.csv("/Users/mutecypher/Documents/Documents - Michael’s iMac/Coursera/Capstone Project/20sample/bi_gram_ns_ns.csv", colClasses = c("NULL", NA, NA) )
combi_bi_ns_ns <- data.table(combi_bi_ns_ns)
trixy <- combi_bi_ns_ns[combi_bi_ns_ns$bi_gram_ns_ns >= 4,]
aggy <- trixy[,.(sum = sum(bi_gram_ns_ns)), by = word2]
