...
word_sen <- tidy_sentences %>% unnest_tokens(word, sentences)
stopit <- stop_words[stop_words$lexicon=="snowball",]
fewer <- sapply(word_sen$word, function(x) gsub(paste(stopit$word, collapse = '|'), '', x))
...
It would take the most frequent words to cover 50% of all word instances of the `r prettyNum(sum(text_count$n), big.mark=",", scientific = FALSE)` words in the sample corpus.

It would take  words to cover 90% of all word instances in the sample corpus.
